...
function Shape( x, y, w, h, zIndex, color, shapeContainer, shadowContainer ) {
  ...
  var _canvasBuffer = new PIXI.CanvasBuffer();
  ...
  var _updateShadowsOnSprite = function( shadowOffsetBasis, shapesArr ) {
    ...
    shapesArr.forEach( function( shape ) {
      ...
      if ( zIndexDiff > 0 ) {
        ...
        // _self sprite.
        var sx = shape.position.x + zIndexDiff * shadowOffsetBasis.x;
        var sy = shape.position.y + zIndexDiff * shadowOffsetBasis.y;
        var dx = sx - _sprite.position.x;
        var dy = sy - _sprite.position.y;
        _ctx.fillStyle = '#20202088'; // grey with 50% alpha
        _ctx.fillRect( dx, dy, shape.getWidth(), shape.getHeight() );
      }
    });
  };
  ...
  var _updateShadowFromSprite = function( shadowOffsetBasis ) {
    _shadow.position.set(
      _sprite.position.x + shadowOffsetBasis.x * _zIndex,
      _sprite.position.y + shadowOffsetBasis.y * _zIndex
     );
  };
